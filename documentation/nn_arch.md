...
- longitude
- direction
- vehicle (splitted into 8 embeddings)
...
The form of the neural network is visualised above. The input layer has 9 neurons, the first hidden layers has 256 neurons, the second hidden layer has 256 neurons, the third hidden layer has 128 neurons, the fourth hidden layer has 64 neurons and the network outputs a single value. The activation function for each hidden layer is *ReLU* and the other activation functions are linear (neutral).